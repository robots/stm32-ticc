...
;   12-Aug-2018  Michal Demin  - adapt to pic12f1572
;
; ------------------------------------------------------------------------
...
; One-time PIC 12F1572 initialization.

init:
        banksel PORTA
        clrf    PORTA           ; set output latches low

        banksel ANSELA
        clrf    ANSELA           ; all digital (no analog) pins

        banksel TRISA
        movlw   ~(1<<RA4|1<<RA1|1<<RA0)
        movwf   TRISA           ; set pin directions (0=output)

        banksel WPUA
        movlw   1<<RA2|1<<RA3
        movwf   WPUA            ; enable weak pullup (1=enable)
...
armed:  movf    PORTA,W          ; GPIO -> W
        andlw   (1<<RA4 | 1<<RA3 | 1<<RA1 | 1<<RA0)
        xorlw   (1<<RA4 |          1<<RA1 | 1<<RA0)
